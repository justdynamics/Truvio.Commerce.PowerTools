...
public sealed class PageAudienceScreen : ListScreenBase<AudienceItemModel>
{
    ...
    protected override Cell? GetCell(string propertyName, AudienceItemModel model) =>
        propertyName switch
        {
            nameof(AudienceItemModel.Visible) => SecOpsBadges.Visible(model.VisibleState, model.Visible),
            nameof(AudienceItemModel.Level) => SecOpsBadges.Level(model.LevelValue, model.Level),
            nameof(AudienceItemModel.Reason) => SecOpsBadges.Origin(model.OriginKind, model.Reason),
            _ => null
        };
}
