    ...
    protected override Cell? GetCell(string propertyName, AccessNodeModel model)
    {
        ...
        return propertyName switch
        {
            nameof(AccessNodeModel.Visible) when model.VisibleState is bool visible =>
                SecOpsBadges.Visible(visible, model.Visible),
            nameof(AccessNodeModel.Level) => SecOpsBadges.Level(model.LevelValue, model.Level),
            nameof(AccessNodeModel.Origin) => SecOpsBadges.Origin(model.OriginKind, model.Origin),
            nameof(AccessNodeModel.Warning) when !string.IsNullOrEmpty(model.Warning) =>
                SecOpsBadges.WarningBadge(model.Warning),
            _ => null
        };
    }
