...
public static class CurrencyGuard
{
        ...
        int deviationPercent)
    {
        ...
        {
            yield return new Finding(NoDefaultId, FindingSeverity.Critical, OperationsEntities.Currency,
        ...
        foreach (var c in currencies.Where(c => c.Rate <= 0))
            yield return new Finding(NonPositiveRateId, FindingSeverity.Critical, OperationsEntities.Currency,
        ...
        foreach (var c in currencies.Where(c => !c.IsDefault && c.Rate > 0 && Near(c.Rate, 100)))
            yield return new Finding(UntouchedRateId, FindingSeverity.Warning, OperationsEntities.Currency,
                c.Code, $"{c.Code} — {c.Name}",
        ...
        int deviationPercent)
    {
        ...
        foreach (var c in currencies.Where(c => !c.IsDefault && c.Rate > 0))
        {
            ...
            yield return new Finding(LiveDeviationId, FindingSeverity.Warning, OperationsEntities.Currency,
                c.Code, $"{c.Code} — {c.Name}",
    ...
    private static void AddStructuralFindings(List<Finding> findings, CurrencySpec defaultCurrency, CurrencySpec context)
    {
        ...
        if (context.Rate <= 0)
            findings.Add(new Finding(NonPositiveRateId, FindingSeverity.Critical, OperationsEntities.Currency,
            ...
            findings.Add(new Finding(UntouchedRateId, FindingSeverity.Warning, OperationsEntities.Currency,
                context.Code, $"{context.Code} — {context.Name}",
                "Exchange rate is exactly 100 — likely never maintained",
    ...
    private static Finding DefaultRateFinding(CurrencySpec defaultCurrency)
    {
        ...
        return new Finding(DefaultRateId, FindingSeverity.Critical, OperationsEntities.Currency,
        ...
        int deviationPercent)
    {
        ...
        foreach (var (code, values) in implied.OrderBy(kv => kv.Key, StringComparer.OrdinalIgnoreCase))
        {
            ...
            yield return new Finding(MatrixDeviationId, FindingSeverity.Warning, OperationsEntities.Currency,
